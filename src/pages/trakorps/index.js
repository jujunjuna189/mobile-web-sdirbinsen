import { bnrBg, icBase, imgMainBg } from "../../assets";
...
const TrakorpsPage = () => {
    const { navigation, satuan, onSearch } = UseTrakorpsContext();
            ...
            <div className="absolute top-0 bottom-0 left-0 right-0 overflow-y-auto">
                ...
                <div className="flex flex-wrap gap-5 px-3 mt-4 justify-center">
                    {satuan?.data?.map((item, index) => {
                        return (
                            <div key={index} className="grow basis-1/3 rounded-md bg-gradient-to-r from-[#A49F1D] via-[#FFFDBC] to-[#CFB20E] p-[0.20rem]" onClick={() => navigation(RouterName.trakorpsDetail, { state: { satuan: { id: item.id } } })}>
