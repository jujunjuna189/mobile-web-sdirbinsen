import axios from "axios";
import { API_PETA_JABATAN_GET, API_PETA_JABATAN_SUMMARY_GET } from "../config/api";

export const getPetaJabatanRequest = async ({ filter = '' }) => {
    // const user = getLocalUser();
    try {

        const response = await axios.get(`${API_PETA_JABATAN_GET}?${filter}`, {
            headers: {
                Authorization: `bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..NmhGNPK-EpJfPUZMxlhl8JUiB2u4_y9K5ozPiM3uxl4`,
            },
        });
        return response.data.list_data.peta_jabatan;
    } catch (error) {
        console.error("Error fetching items:", error);
    }
};

export const getPetaJabatanSummaryRequest = async ({ filter = '' }) => {
    // const user = getLocalUser();
    try {

        const response = await axios.get(`${API_PETA_JABATAN_SUMMARY_GET}?${filter}`, {
            headers: {
                Authorization: `bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..NmhGNPK-EpJfPUZMxlhl8JUiB2u4_y9K5ozPiM3uxl4`,
            },
        });
        return response.data.list_data.peta_jabatan;
    } catch (error) {
        console.error("Error fetching items:", error);
    }
};